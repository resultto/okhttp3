...
public final class StreamAllocation {
    ...
    private RealConnection findHealthyConnection(int connectTimeout, int readTimeout,
        ...
        while (true) {
            ...
            //如果是一个全新的连接，跳过检查，直接返回。
            synchronized (connectionPool) {
                if (candidate.successCount == 0) {
                    return candidate;
                }
            }
            //如果是一个不好使的连接，移除连接池。一直持续。
    ...
    private Socket deallocate(boolean noNewStreams, boolean released, boolean streamFinished) {
        ...
        if (connection != null) {
            if (noNewStreams) {
                //如果设置为true,则这个连接就不会创建新的stream，一旦设置为true,就一直是true。
                //该属性的设置地方如下，evitAll，关闭或移除连接池中的所有空闲连接，则noNewStrams为true。
                //pruneAndGetAllocationCount,移除内存泄露的连接及获取连接的Stream分配数。
                //streamFailed，Stream分配失效。
                //禁止无效连接创建新的straem。
                connection.noNewStreams = true;
            }
            if (this.codec == null && (this.released || connection.noNewStreams)) {
                release(connection);//释放Connetion承载的StreamAllocations资源
                if (connection.allocations.isEmpty()) {
                    connection.idleAtNanos = System.nanoTime();
                    //通知线程池该连接是空闲连接，可以移除或者作为待移除对象。
